...
export function ActionItemsPage() {
  ...
  if (loading) return <PageShell title="Action Items"><Spinner /></PageShell>;
  if (error) return <PageShell title="Action Items"><ErrorMsg msg={error} /></PageShell>;
  ...
  return (
    <PageShell title="Action Items" count={items.length}>
      {items.map((item) => (
        <Card key={item.id} style={{ marginBottom: 10, display: "flex", gap: 14, alignItems: "flex-start" }}>
          {item.status === "done"
